...
const MakerForm: React.FC = () => {
    ...
    const [status, setStatus] = useState<MakerStatusEnum>(
        MakerStatusEnum.ACTIVE
    );

    // Contatos podem ter ID se vierem da edição
    ...
    const fetchMakerData = async (makerId: string) => {
        try {
            ...
            setSelectedCategories(
                new Set(makerData.categories.map((cat) => cat.id))
            );
    ...
    const handleContactChange = (
        ...
        // @ts-ignore - typescript reclama da tipagem estrita aqui, mas é seguro
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // CORREÇÃO: Sanitizar contatos (remover ID)
            ...
            const makerData = {
                ...
                status, // Agora o Backend aceita status
            ...
            if (isEditing && id) {
                ...
                if (fileToUpload)
                    await uploadMakerProfileImage(id, fileToUpload);
            } else {
                // Nota: Criar maker pelo Admin pode falhar se o backend exigir firebaseUid.
                // Idealmente o cadastro é feito pelo fluxo público.
            ...
            // Tratamento de erro melhorado (array de mensagens)
                        ...
                        <div className="space-y-2">
                            {contacts.map((contact, index) => (
                                ...
                                >
                                    ...
                                    >
                                        {Object.values(ContactTypeEnum).map(
                                            (type) => (
                        ...
                        {isEditing && profileImage && (
                            <div className="mb-4 relative w-40 h-40 group">
                                ...
                                <button
                                    type="button"
                                    onClick={() =>
                                        handleImageDelete(profileImage.id)
                                    }
                    ...
                    <section>
                        <div>
                            ...
                            <select
                                ...
                                value={status}
                                onChange={(e) =>
                                    setStatus(e.target.value as MakerStatusEnum)
                                }
