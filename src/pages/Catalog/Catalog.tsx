...
// --- PASSO 2 (EXTRA): Ícone para o novo campo de busca ---
const UserIcon = () => ( <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}> <path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /> </svg> );

// --- Gerador de Dados de Exemplo ---
const generateMockProducts = (count) => {
    // ... (seu código de geração de dados continua igual)
    const products = [];
    const categories = ["Action Figure", "Boneco", "Funko Pop", "Diorama", "Miniatura", "Cosplay", "Acessórios"];
    const titles = ["Herói das Sombras", "Guerreiro Estelar", "Criatura Mística", "Robô de Batalha", "Mago Poderoso", "Elmo Lendário", "Amuleto Antigo"];
    const makers = [
        { name: "Ana Silva", storeName: "Ana Artes 3D", location: "São Paulo, SP", rating: 4.8, productCount: 52, bio: "Especialista em action figures detalhadas.", tags: ["Action Figures", "Colecionáveis"], offersCustomization: true, whatsapp: "5511912345678", instagram: "anaartes3d", featuredProduct: { imageUrl: "https://placehold.co/100x100/3E3E3E/FFFFFF?text=Heroi", title: "Herói Sombrio", description: "Peça de colecionador.", price: "250.00" } },
        { name: "Bruno Costa", storeName: "BC Prints", location: "Rio de Janeiro, RJ", rating: 4.9, productCount: 35, bio: "Dioramas e cenários realistas.", tags: ["Dioramas", "RPG"], offersCustomization: false, featuredProduct: { imageUrl: "https://placehold.co/100x100/3E3E3E/FFFFFF?text=Diorama", title: "Cenário Medieval", description: "Perfeito para jogos de RPG.", price: "350.00" } },
        { name: "Carla Dias", storeName: "Pop Collectibles", location: "Belo Horizonte, MG", rating: 4.7, productCount: 88, bio: "A maior coleção de Funkos customizados.", tags: ["Funko Pop", "Cultura Pop"], offersCustomization: true, instagram: "carlapops", featuredProduct: { imageUrl: "https://placehold.co/100x100/3E3E3E/FFFFFF?text=Funko", title: "Funko Custom", description: "Seu personagem favorito.", price: "180.00" } },
    ];
...
const Catalog: React.FC = () => {
    ...
    // --- PASSO 1: Adicionar um novo estado para a busca por maker ---
    ...
    useEffect(() => {
        if (selectedMaker) { document.body.style.overflow = 'hidden'; } 
    ...
    const categoryCounts = useMemo(() => {
        ...
        mockProducts.forEach(p => { counts[p.category] = (counts[p.category] || 0) + 1; });
        return counts;
    }, []);
    ...
    const filteredAndSortedProducts = useMemo(() => {
        ...
        // --- PASSO 3: Adicionar a lógica de filtro por nome do maker ---
        if (makerSearchInput) { 
            result = result.filter(p => p.maker.name.toLowerCase().includes(makerSearchInput.toLowerCase())); 
        }
        ...
        return result;
    // --- PASSO 3 (continuação): Adicionar makerSearchInput à lista de dependências ---
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                    <aside className="lg:col-span-1">
                        <div className="bg-gray-100 dark:bg-[#1a1a1a] rounded-2xl p-6 sticky top-28">
                            <h2 className="flex items-center gap-3 text-2xl font-bold text-gray-900 dark:text-white mb-6"> <FilterIcon /> Categorias </h2>
                    ...
                    <main className="lg:col-span-3">
                        {/* --- PASSO 2: Adicionar o novo campo de busca por maker --- */}
                        <div className="flex flex-col md:flex-row gap-6 justify-between items-center mb-8">
                           <div className="relative flex-grow w-full">
                               <span className="absolute inset-y-0 left-0 flex items-center pl-4"><SearchIcon /></span>
                               <input type="text" placeholder="Buscar produtos..." value={searchInput} onChange={e => setSearchInput(e.target.value)} className="bg-gray-100 dark:bg-[#1a1a1a] w-full border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white rounded-lg py-3 pl-14 pr-4 text-lg focus:outline-none focus:ring-2 focus:ring-purple-500" />
                           </div>
                           <div className="relative flex-grow w-full">
                               <span className="absolute inset-y-0 left-0 flex items-center pl-4"><UserIcon /></span>
                               <input type="text" placeholder="Buscar por maker..." value={makerSearchInput} onChange={e => setMakerSearchInput(e.target.value)} className="bg-gray-100 dark:bg-[#1a1a1a] w-full border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white rounded-lg py-3 pl-14 pr-4 text-lg focus:outline-none focus:ring-2 focus:ring-purple-500" />
                           </div>
                           <CustomSelect options={sortOptions} value={sortBy} onChange={(value) => setSortBy(value)} />
                        </div>

                        <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 ${animateGrid ? styles.gridFadeIn : ''}`}>
                            {productsToShow.map((product, index) => {
                                ...
                                if (productsToShow.length === index + 1) { return <div ref={lastProductElementRef} key={product.id}>{card}</div>; }
                                return card;
                            })}
                        </div>
